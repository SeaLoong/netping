...
pub fn start(addr: SocketAddr, size: usize, timeout: u64) {
    ...
    loop {
        ...
        thread::spawn(move || {
            ...
            println!(
                "{} bytes received from {} through single package, remain {} bytes",
                bytes,
                client_addr,
                size - bytes
            );
            ...
            println!(
                "{} bytes sent to {} through single package, remain {} bytes",
                bytes,
                client_addr,
                size - bytes
            );
